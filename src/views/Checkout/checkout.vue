    ...
    <div class="w" style="padding-top: 40px;">
        ...
        <div slot="content">
          <ul class="address-item-list clearfix">
            <li v-for="(item,i) in addList"
                ...
                @click="chooseAddress(item.id, item.receiverName, item.receiverTelephone, item.receiverAddress)">
           <span v-if="id === item.id" class="pa">
             ...
             <path
              ...
              <p>收货人: {{item.receiverName}} {{item.isDefault === '1' ? '(默认地址)' : ''}}</p>
              <p class="street-name ellipsis">收货地址: {{item.receiverAddress}}</p>
              <p>手机号码: {{item.receiverTelephone}}</p>
      ...
      <y-popup :open="popupOpen" @close='popupOpen=false' :title="popupTitle">
        <div slot="content" class="md" :data-id="msg.id">
          ...
          <div>
            <input type="number" placeholder="手机号码" v-model="msg.receiverTelephone">
          </div>
          <div>
          ...
          <y-button text='保存'
                    ...
                    @btnClick="save({userId:userId,id:msg.id,receiverName:msg.receiverName,receiverTelephone:msg.receiverTelephone,receiverAddress:msg.receiverAddress,isDefault:msg.isDefault ? 1 : 0})">
          </y-button>
        </div>
      </y-popup>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      btnHighlight () {
        let msg = this.msg
        return msg.receiverName && msg.receiverTelephone && msg.receiverAddress
      },
      // 选中的总价格
      checkPrice () {
        let totalPrice = 0
    ...
    methods: {
      ...
      _addressList (params) {
        addressList(params).then(res => {
          ...
          if (data.length) {
            ...
            this.receiverTelephone = data[0].receiverTelephone
      ...
      _addressUpdate (params) {
        addressUpdate(params).then(res => {
          this._addressList({params: {memberId: this.userId}})
        })
      },
      _addressAdd (params) {
      ...
      // 选择地址
      chooseAddress (id, receiverName, receiverTelephone, receiverAddress) {
        this.id = id
      ...
      update (item) {
        ...
        if (item) {
          ...
          this.msg.receiverTelephone = item.receiverTelephone
          this.msg.receiverAddress = item.receiverAddress
          // 1是默认地址
          this.msg.isDefault = (item.isDefault === 1)
          this.msg.id = item.id
        } else {
          this.popupTitle = '新增收货地址'
          this.msg.receiverName = ''
          this.msg.receiverTelephone = ''
